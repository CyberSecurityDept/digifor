...
```bash
# Health check
curl http://172.15.1.207/health

# Login
curl -X POST "http://172.15.1.207/api/v1/auth/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "username=admin&password=admin123"

# Create case (ganti YOUR_TOKEN dengan token dari login)
curl -X POST "http://172.15.1.207/api/v1/cases/" \
...
2. **Explore Documentation** di http://172.15.1.207/docs
...
2. Check health endpoint: http://172.15.1.207/health
